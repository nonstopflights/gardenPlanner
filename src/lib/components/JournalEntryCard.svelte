<script lang="ts">
	...
	function handleContentClick(e: MouseEvent) {
		// Don't fire if a button was clicked inside
		...
		>
			<div class="flex items-start justify-between gap-3">
				...
				>
					...
					{#if entry.content}
						<p class="mt-1 line-clamp-2 text-sm text-slate-500">{truncate(stripHtml(entry.content), 120)}</p>
					{/if}
					{#if images.length > 0}
		...
		>
			<div class="flex items-start justify-between gap-3">
				...
				>
					...
					{#if entry.content}
						<p class="mt-2 line-clamp-3 text-sm text-slate-600">{truncate(stripHtml(entry.content), 280)}</p>
					{/if}
					{#if images.length > 0}
